...
Las hojas del template contienen la información necesaria para generar el certificado. La hoja `Especificaciones` contiene la información para obtener datos de los elementos a calibrar y en el futuro, información para generar la carátula del certificado. Los campos de cada elemento se traducen a pares `key:value` donde el key es la etiqueta de la columna A (tildes eliminadas y espacios reemplazados por `_`) y el value es el texto de la columna B. En las hojas que se agregan mediante la función `add_sections`, existen varios comportamientos que se configuran mediante el contenido de las columnas de cada fila:
- Texto simple: Se imprime el texto de la columna A en la hoja.
- Texto único con condición: Se imprime el texto de la columna A en la hoja si el par `key:value` de la columna C se cumple. La columna B debe contener '**'. El texto en A puede contener `*texto_singular:texto_plural*` para modificar su comportamiento segun la cantidad de veces que se cumple la condición. Además, puede contener `$key$` para completar con información general. Es posible definir keys personalizados con funciones específicas que operen sobre los elementos a calibrar. En el caso de querer referenciar tablas o figuras, se realiza usando un key con el formato `_tab:tabla_name` o `_fig:figura_name`, o poniendo `tab_next`, `tab_this`, o `tab_prev` (idem para figuras).
...
- Imagen: Se imprime la imagen con el nombre de archivo dado en la columna B (ubicado en la carpeta `resources/Certificados`). El texto de la columna A se imprime debajo de la imagen, pero debe comenzar con `_fig`, lo cual se ignora al imprimir. La columna C contiene el ancho de la imagen en mm.
    - Figura con subfiguras: La figura se divide en subfiguras segun la cantidad de `(filas, columnas)` expresadas en la columna B. El texto de la columna A puede contener `$key$` para modificar su comportamiento según el elemento procesado. La columna D contiene el par `key:value` de condición para procesar el elemento.
- Tabla: Se imprime la tabla con el label dado por el comienzo `_tab:tabla_name` y el caption dado en la columna A. La columna B contiene el par `key:value` de condición para procesar la tabla. Se debe proporcionar una lista de DataFrames para su generación en add_sections.
