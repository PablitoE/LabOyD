...
def test_find_equidistant_peaks():
    ...
    true_peaks_locs = np.arange(20, 180, 10)
    true_peaks_values = np.linspace(1.5, 3, len(true_peaks_locs)) + 3
    ...
    peaks = fei.find_equidistant_peaks(data, max_min='max', prominence=1)     # , distance=5
    plt.plot(data)
